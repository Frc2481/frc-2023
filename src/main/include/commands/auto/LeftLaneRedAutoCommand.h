...
frc2::CommandPtr LeftLaneRedAutoCommand(Drivetrain* m_pDrive, Elevator* m_pElevator, Flipper* m_pFlipper, Gripper* m_pGripper, Intake* m_pIntake, Slide* m_pSlide) {

  
    frc::Pose2d m_initialPosition = frc::Pose2d{0_in, 0_in, 0_deg};

    frc::TrajectoryConfig forwardConfig{units::velocity::feet_per_second_t(RobotParameters::k_maxSpeed / 3),
                                 units::acceleration::feet_per_second_squared_t(RobotParameters::k_maxAccel / 3)};
    forwardConfig.SetKinematics(m_pDrive->GetKinematics());
    forwardConfig.SetReversed(false);

    frc::TrajectoryConfig reverseConfig{units::velocity::feet_per_second_t(RobotParameters::k_maxSpeed / 3),
                                 units::acceleration::feet_per_second_squared_t(RobotParameters::k_maxAccel / 3)};
    reverseConfig.SetKinematics(m_pDrive->GetKinematics());
    reverseConfig.SetReversed(true);

  
    frc::TrajectoryConfig reverseChargeStationApproachConfig{units::velocity::feet_per_second_t(RobotParameters::k_maxSpeed / 3),
                                 units::acceleration::feet_per_second_squared_t(RobotParameters::k_maxAccel / 3)};
